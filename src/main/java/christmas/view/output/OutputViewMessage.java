package christmas.view.output;

public class OutputViewMessage {
    public static final String EVENT_BENEFIT_PREVIEW_MESSAGE = "12월 %s일에 우테코 식당에서 받을 이벤트 혜택 미리 보기!%n" + '\n';
    public static final String ORDER_MENU_MESSAGE = "<주문 메뉴>";
    public static final String TOTAL_ORDER_AMOUNT_BEFORE_DISCOUNT_MESSAGE = '\n' + "<할인 전 총주문 금액>";
    public static final String GIVE_AWAY_MENU_MESSAGE = '\n' + "<증정 메뉴>";
    public static final String BENEFIT_DETAILS_MESSAGE = "<혜택 내역>";
    public static final String TOTAL_BENEFIT_AMOUNT_MESSAGE = "<총혜택 금액>";
    public static final String ESTIMATED_PAYMENT_AMOUNT_AFTER_DISCOUNT_MESSAGE = '\n' + "<할인 후 예상 결제 금액>";
    public static final String DECEMBER_EVENT_BADGE_MESSAGE = '\n' + "<12월 이벤트 배지>";
}
