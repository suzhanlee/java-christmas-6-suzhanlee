...
public class ChristmasDDayDiscountPolicy implements DiscountPolicy {
    ...
    public ChristmasDDayDiscountPolicy(LocalDate localDate) {
        this.localDate = localDate;
    }

    public boolean supports() {
        return localDate.getDayOfMonth() <= TWENTY_FIVE_DAY;
    }

    public Map<String, Long> calculateDiscountAmount(Menus menus) {
        Map<String, Long> benefitDetail = new HashMap<>();
        long discountAmount =
                DEFAULT_CHRISTMAS_D_DAT_DISCOUNT_AMOUNT + (localDate.getDayOfMonth() - 1) * CHRISTMAS_INCREMENT;
